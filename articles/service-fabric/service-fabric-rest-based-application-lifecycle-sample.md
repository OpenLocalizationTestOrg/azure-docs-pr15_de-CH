<properties
   pageTitle="REST-basierten Lifecycle Anwendungsbeispiel | Microsoft Azure"
   description="Eine Microsoft Azure Service Fabric Beispiel Anwendungslebenszyklus mithilfe der Service Fabric REST-Schnittstelle."
...
# <a name="rest-based-application-lifecycle-sample"></a>REST-basierten Lifecycle Anwendungsbeispiel

Dieses Beispiel demonstriert Anwendungslebenszyklus Service Fabric über REST API-Aufrufe. Weitere Informationen zu Service Fabric-Anwendungslebenszyklus finden Sie unter [Service Fabric-Anwendungslebenszyklus](service-fabric-application-lifecycle.md).

Dieses Beispiel führt Folgendes aus:

* Vorschriften **1.0.0 WordCount** -Beispiel aus dem Anwendungspaket WordCount im Bild speichern.
* Zeigt die Liste der Anwendungstypen, einschließlich WordCount 1.0.0.
* Erstellt die WordCount-Anwendung als **Fabric: / WordCount**.
* Zeigt die Liste der Programme, einschließlich Fabric: / WordCount Version 1.0.0.
* Vorschriften der 1.1.0 Version WordCount-Beispiel aus dem Anwendungspaket **WordCountUpgrade** im Bild Speicher.
* Zeigt die Liste der Anwendungstypen, einschließlich WordCount 1.0.0 und **WordCount 1.1.0**.
* WordCount-Anwendung auf Version 1.1.0 aktualisiert.
* Zeigt die Liste der Programme, die WordCount Version 1.1.0, aber enthält keine WordCount Version 1.0.0.
* Löscht die WordCount-Anwendung.
* Zeigt die Liste der Programme, mehr umfasst Fabric: / WordCount.
* Unprovisions der 1.1.0 Version WordCount-Beispiel.
* Zeigt die Liste der Anwendungstypen, die WordCount 1.0.0, aber enthält keine WordCount 1.1.0.
* Unprovisions der 1.0.0 Version WordCount-Beispiel.
* Zeigt die Liste der Anwendungstypen, die WordCount mehr enthält.


## <a name="prerequisites"></a>Erforderliche Komponenten

Dieses Beispiel verwendet die [WordCount-Beispiel](http://aka.ms/servicefabricsamples) (in den Beispielen **Einstieg** gefunden). WordCount-Beispiel muss zuerst erstellt werden, und zwei Anwendungspakete Image-Speicher kopiert werden müssen.

|Ordner|Beschreibung|
|------|-----------|
|WordCount|WordCount-beispielanwendung. Die Datei **ApplicationManifest.xml** **ApplicationTypeVersion = "1.0.0"**.|
|WordCountUpgrade|WordCount-beispielanwendung. ApplicationManifest.xml Datei muss geändert werden, um **ApplicationTypeVersion = "1.1.0"** ermöglicht die Aktualisierung der Anwendung auftreten.|

Erstellen der Anwendung in der Abbildspeicher kopieren und folgende Schritte ausführen:

1. Kopieren Sie **C:\ServiceFabricSamples\Services\WordCount\WordCount\pkg\Debug** in **C:\Temp\WordCount**. Dadurch wird das Anwendungspaket WordCount erstellt.
2. Kopieren Sie C:\Temp\WordCount in **C:\Temp\WordCountUpgrade**. Paket **WordCountUpgrade-Anwendung** wird erstellt.
3. Öffnen Sie **C:\Temp\WordCountUpgrade\ApplicationManifest.xml** in einem Texteditor.
4. Ändern Sie das **ApplicationTypeVersion** -Attribut im **ApplicationManifest** Element in **"1.1.0"**.  Dadurch wird die Versionsnummer der Anwendung aktualisiert.
5. Speichern Sie die geänderte Datei ApplicationManifest.xml.
6. Führen Sie das folgende PowerShell-Skript als Administrator der Anwendung der Abbildspeicher kopieren:
...
PowerShell-Skript abgeschlossen ist, wird diese Anwendung ausführen.

## <a name="example"></a>Beispiel

Das folgende Beispiel veranschaulicht des Anwendungslebenszyklus Service Fabric.
