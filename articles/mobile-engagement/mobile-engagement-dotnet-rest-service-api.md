<properties 
    pageTitle="Die REST-API verwenden auf Azure Mobile Engagement Service-APIs" 
    description="Beschreibt, wie Mobile Engagement REST APIs auf Azure Mobile Engagement Service-APIs"       
...
#<a name="using-rest-to-access-azure-mobile-engagement-service-apis"></a>Verwendung von REST auf Azure Mobile Engagement Service-APIs

Azure Mobile Engagement bietet [Azure Mobile Engagement REST-API](https://msdn.microsoft.com/library/azure/mt683754.aspx) zur Verwaltung von Geräten, Reach-Push-Kampagnen usw.. In diesem Beispiel verwenden die REST-APIs direkt zum Erstellen einer Ankündigung Kampagne aktivieren und entscheiden Sie sich für eine Reihe von Geräten. 

Wenn Sie nicht die REST-APIs direkt verwenden möchten, bieten wir auch eine [Datei Swagger](https://github.com/Azure/azure-rest-api-specs/blob/master/arm-mobileengagement/2014-12-01/swagger/mobile-engagement.json) mit Anwendungsmöglichkeiten SDKs für Ihre bevorzugte Sprache generieren. Wir empfehlen [AutoRest](https://github.com/Azure/AutoRest) Tool Ihren SDK aus unserer stolz-Datei generiert. Haben wir .NET SDK ähnlich der Interaktion mit diesen APIs C#-Wrapper verwenden können, und Sie müssen token Authentifizierungsverhandlungen und selbst aktualisieren. Ein ähnliches Beispiel mit dieser Wrapper durchlaufen, finden Sie unter [Service API .NET SDK-Beispiel](mobile-engagement-dotnet-sdk-service-api.md)

Dieses Beispiel verwendet die REST-APIs direkt erstellen und Aktivieren einer Ankündigung Kampagne. 

## <a name="adding-a-username-appinfo-to-the-mobile-engagement-app"></a>Mobile Engagement app hinzufügen User_name appInfo

Dieses Beispiel erfordert ein app-Info-Tag app Mobile Engagement hinzugefügt. Sie können das Tag im Portal Engagement für Ihre Anwendung hinzufügen, **Klicken Sie** > **Tag (app-Info)** > **Neues Tag (app-Info)**. Fügen Sie das neue Tag namens **Benutzername** als **Zeichenfolgentyp** .

![](./media/mobile-engagement-dotnet-rest-service-api/user-name-app-info.png)



Wenn Sie [Einstieg in Azure Mobile Engagement für Windows Universal Apps](mobile-engagement-windows-store-dotnet-get-started.md)ausgeführt, Sie können testen **User_name** Tag durch eine einfache Überschreibung `OnNavigatedTo()` in der `MainPage` Klasse an der app Info Tag ähnelt dem folgenden Code:
...
## <a name="creating-the-service-api-app"></a>Die Dienst-API-Anwendung erstellen

1. Zunächst benötigen Sie vier Authentifizierungsparameter mit diesem Beispiel. Diese Parameter sind **SubscriptionId**, **TenantId** **ApplicationId** und **Schlüssel**. Zu diesen Authentifizierungsparameter empfiehlt es sich, den PowerShell-Skript Ansatz Abschnitt *einmalige Einrichtung (mithilfe von Skript)* im Lernprogramm [Authentifizierung](mobile-engagement-api-authentication.md#authentication) verwenden. 

2. Wir verwenden eine einfache Windows-Konsole app mit der REST-Dienst-APIs erstellen und Aktivieren einer neuen Ankündigung Kampagne veranschaulicht. So öffnen Sie Visual Studio, und erstellen Sie ein neues **Konsolenanwendungsprojekt**.   

3. Fügen Sie **Newtonsoft.Json** NuGet-Paket hinzu.

4. In der `Program.cs` Datei, fügen Sie den folgenden `using` -Anweisung für die folgenden Namespaces:
...
5. Als Nächstes müssen Sie die folgenden Konstanten in der `Program` Klasse. Diese werden zur Authentifizierung und Interaktion mit Mobile Engagement App in der Ankündigung Kampagne erstellen:
...
6. Fügen Sie die beiden folgenden Methoden die `Program` Klasse. Diese dienen REST-APIs und Antworten auf der Konsole angezeigt.
...
7. Fügen Sie den folgenden Code in die `Main` -Methode ein Authentifizierungstoken mit Authentifizierungsparameter generieren Sie gesammelt:
...
8. Jetzt haben wir einen gültigen Authentifizierungstokens können wir eine neue Reach-Kampagne mit der [Kampagne erstellen](https://msdn.microsoft.com/library/azure/mt683742.aspx) REST-API erstellen. Neue Kampagne werden eine einfache **jederzeit** & **Benachrichtigung nur** Ankündigung Kampagne mit Titel angezeigt. Eine manuelle pushkampagne werden wie im folgenden Screenshot gezeigt. Dies bedeutet, dass es nur geschoben werden mithilfe der APIs.


    ![](./media/mobile-engagement-dotnet-rest-service-api/manual-push.png)

    Fügen Sie den folgenden Code in die `Main` -Methode die Ankündigung Kampagne erstellen: 
...
9. Die Kampagne muss aktiviert werden, bevor es Geräten abgelegt werden kann. Wir speichern die ID für die neue Kampagne in der `NewCampaignID` Variable. Wir verwenden dies als einen Pfadparameter URI die Kampagne [Aktivieren Kampagne](https://msdn.microsoft.com/library/azure/mt683745.aspx) REST-API aktivieren. Dies sollte der Kampagne **berechneten** Zustandsänderung, obwohl sie nur mit den APIs manuell verschoben werden.

    Fügen Sie den folgenden Code in der `Main` -Methode die Ankündigung Kampagne: 
...
10. Um die Kampagne müssen Geräte-Ids für Benutzer bereitstellen, der die Benachrichtigung erhalten soll. Geräte-ID abrufen, verwenden Sie die [Geräte Abfragen](https://msdn.microsoft.com/library/azure/mt683826.aspx) REST-API. Wir werden jedes Id zur Liste hinzufügen, wenn **Benutzername** AppInfo zugeordnet sind.

    Fügen Sie den folgenden Code in die `Main` -Methode alle Geräte-Ids und füllen die Geräteliste:
...
11. Schließlich werden wir die Kampagne an alle Geräte-Ids in unserer Liste mit [Push-Kampagne](https://msdn.microsoft.com/library/azure/mt683734.aspx) REST API forcieren. Dies ist eine Benachrichtigung **in app** . So haben die Anwendung auf dem Gerät zu empfangen vom Benutzer ausgeführt werden.

    Fügen Sie den folgenden Code in die `Main` -Methode müssen die Campign der Geräte in der Geräteliste:
...
12. Erstellen und Ausführen der Konsole app. Die Ausgabe sollte ähnlich der folgenden:
