...
Dieser Artikel beschreibt, wie mithilfe die Service Fabric-Lösung in Protokollanalyse erkennen und Beheben von Problemen über Cluster Service Fabric, Sichtbarkeit wie immer Service Fabric-Knoten führen und wie die Applikationen und Micro-Dienste ausgeführt werden.

Service Fabric-Lösung verwendet Azure-Diagnose Daten aus Ihrem Service Fabric VMs durch Erfassung von Daten aus Tabellen Azure Bündel. Anschließend liest Protokollanalyse Fabric Service Framework Ereignisse, einschließlich der **zuverlässigen Service**, **Akteur Ereignisse**, **Operationelle Ereignisse**, und **benutzerdefinierte ETW-Ereignisse**. Service Fabric lösungsdashboard zeigt wichtige Probleme und entsprechende Ereignisse in Ihrer Umgebung Service Fabric.

## <a name="installing-and-configuring-the-solution"></a>Installation und Konfiguration der Lösung

Führen Sie diese drei einfachen Schritte zur Installation und Konfiguration der Lösung:

1. Sicherstellen Sie, dass OMS-Arbeitsbereich, den Verwendung derselben Azure-Abonnement zugeordnet ist, mit der alle Clusterressourcen mit Storage-Konten erstellen. Finden Sie Informationen zum Erstellen eines OMS-Arbeitsbereichs [Protokollanalyse Einstieg](log-analytics-get-started.md) .
2. Konfigurieren Sie OMS zum Sammeln und Anzeigen von Service Fabric-Protokolle.
3. Aktivieren der Service Fabric-Lösung im Arbeitsbereich.

## <a name="configure-oms-to-collect-and-view-service-fabric-logs"></a>Konfigurieren Sie OMS zum Sammeln und Anzeigen von Service Fabric-Protokolle
In diesem Abschnitt lernen Sie OMS abrufen Service Fabric-Protokolle konfigurieren. Die Protokolle können anzeigen, analysieren und Fehlerbehebung im Cluster oder in Programme und Dienste in diesem Cluster mit OMS-Portal.

>[AZURE.NOTE] Die Azure-Diagnose-Erweiterung muss zum Hochladen der Protokolle auf Tabellen, die übereinstimmen OMS sieht für konfiguriert werden. Finden Sie unter [Sammeln mit Azure-Diagnose](../service-fabric/service-fabric-diagnostics-how-to-setup-wad.md) Weitere Protokolle sammeln. Configuration Settings Beispiele in diesem Artikel zeigen, welche die Namen der Tabellen werden soll. Diagnose auf dem Cluster eingerichtet und Protokolle auf ein Speicherkonto hochlädt, besteht der nächste Schritt konfiguriert OMS um diese Protokolle zu sammeln.

Stellen Sie sicher, Abschnitt **EtwEventSourceProviderConfiguration** in der **template.json** -Datei Einträge für die neue Recording, bevor Sie die Aktualisierung der Konfiguration anwenden, mit **deploy.ps1**zu aktualisieren. Die Tabelle für den Upload entspricht (ETWEventTable). Im Moment können OMS Anwendung ETW-Ereignisse aus dieser Tabelle schreibgeschützt. Jedoch Unterstützung für benutzerdefinierte ETW-Tabellen in der Entwicklung.

Zum Ausführen einiger Vorgänge in diesem Abschnitt werden die folgenden Tools verwendet:

-   Azure PowerShell
-   [Operations Management Suite](http://www.microsoft.com/oms)

### <a name="configure-an-oms-workspace-to-show-the-cluster-logs"></a>Konfigurieren Sie einen OMS-Arbeitsbereich, um die Cluster Protokolle

Nachdem ein Arbeitsbereich OMS wie oben beschrieben erstellt haben, besteht der nächste Schritt den Arbeitsbereich ziehen die Protokolle der Azure-Speicher Tabellen konfigurieren, in dem sie aus dem Cluster Diagnostics Erweiterung hochgeladen werden. Dazu führen Sie das folgende PowerShell-Skript:
...
Nachdem Sie konfiguriert haben OMS Arbeitsbereich Lesen von Azure Tabellen das Speicherkonto Azure-Portal anmelden und **Alle Ressourcen**der OMS-Arbeitsbereich auswählen. Wenn ausgewählt, erhalten Sie die Anzahl der Speichergruppen Konto anmeldet, OMS-Arbeitsbereich verbunden. Wählen Sie **Konto Speicherprotokollen** Kachel, und überprüfen Sie aus der Liste der Protokolle von Speichergruppe Konto das Speicherkonto OMS Arbeitsbereich verbunden ist:

![Protokolle von Speichergruppe Konto](./media/log-analytics-service-fabric/sf1.png)

## <a name="enable-the-service-fabric-solution"></a>Das Fabric Service Lösung
Verwenden Sie das folgende Skript OMS Arbeitsbereich der Projektmappe hinzu. Führen Sie das Skript in PowerShell mit OMS-Arbeitsbereich zugeordnet ist, die den Service Fabric Lösung in Azure-Abonnement.
...
>[AZURE.NOTE] Auf oben das Dashboard **basierend auf 7 Tagen** können Sie den Bereich dieser Ereignisse in Service Fabric-Lösung ändern. Sie können auch Ereignisse innerhalb der letzten 7 Tage, 1 Tag oder 6 Stunden anzeigen. Oder Sie können **benutzerdefinierte** benutzerdefinierten Datumsbereich an.


## <a name="troubleshoot-your-service-fabric-and-oms-configuration"></a>Problembehandlung bei der Service Fabric und OMS-Konfiguration

Möchten Sie die OMS-Konfiguration überprüfen, da Sie in OMS Daten anzeigen können, verwenden Sie das folgende Skript. Liest Service Fabric Diagnosekonfiguration überprüft, ob Daten in die Tabellen geschrieben und überprüft Tabellen gelesen OMS konfiguriert ist.
